...
export function DistributorManagerInvoiceGenerator() {
  const { userData } = useAuth();
  const { getUserStockSummary, useStock, loading, error } = useDistributorStockOperations();
  ...
  const generateInvoiceNumber = () => {
    ...
    return `DIST-INV-${timestamp}-${random}`;
  };
  ...
  const handleReleaseInvoice = async (finalInvoiceData: InvoiceData) => {
    ...
    try {
      ...
      const invoiceRecord = {
        ...
        distributorEmail: userData.email,
