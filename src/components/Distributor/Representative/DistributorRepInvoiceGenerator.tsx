...
export function DistributorRepInvoiceGenerator() {
  const { userData } = useAuth();
  const { getUserStockSummary, useStock, loading, error } = useDistributorRepStockOperations();
  ...
  const updateItemQuantity = (index: number, quantity: number) => {
    ...
    const maxQuantity = stock.availableQuantity;
  ...
  const generateInvoiceNumber = () => {
    ...
    return `DISTREP-INV-${timestamp}-${random}`;
  };
  ...
  const handleReleaseInvoice = async (finalInvoiceData: InvoiceData) => {
    ...
    try {
      ...
      const invoiceRecord = {
        ...
        distributorId: userData.distributorId || 'unknown',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          {error && <ErrorMessage message="Failed to load stock" />}
