...
export function DRInvoiceGenerator() {
  ...
  const { getUserStockSummary, useStock, loading, error } = useRoleStockOperations('DirectRepresentative');
  ...
  const loadStockData = async () => {
    ...
    try {
      console.log('[DRInvoiceGenerator] Loading stock for user:', userData.id);
      console.log('[DRInvoiceGenerator] User role:', userData.role);
      const summary = await getUserStockSummary(userData.id);
      console.log('[DRInvoiceGenerator] Stock summary received:', summary);
      const availableStock = summary.filter(item => item.availableQuantity > 0);
      console.log('[DRInvoiceGenerator] Available stock (filtered):', availableStock);
      setStockSummary(availableStock);
    } catch (err: any) {
      console.error('[DRInvoiceGenerator] Error loading stock:', err);
      console.error('[DRInvoiceGenerator] Error message:', err?.message);
      console.error('[DRInvoiceGenerator] Error stack:', err?.stack);
    }
  };
  ...
  const generateInvoiceNumber = () => {
    ...
    return `DR-INV-${timestamp}-${random}`;
  };
  ...
  const handleReleaseInvoice = async (finalInvoiceData: InvoiceData) => {
    ...
    try {
      for (const item of finalInvoiceData.items) {
        await useStock(userData.id, item.productId, item.quantity, `Sold via invoice ${finalInvoiceData.invoiceNo}`);
      }

      const invoiceRef = ref(database, `drinvoices/${userData.id}`);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Select products from your stock and create customer invoices</p>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <ErrorMessage message={`Stock loading error: ${error}`} />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">Click on a product to add it to the invoice</p>
          </div>
            ...
            <div className="p-4 space-y-2 max-h-[600px] overflow-y-auto">
              {stockSummary.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-500 mt-1">Your inventory is empty. Request stock to get started.</p>
                </div>
              ) : (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="pt-2 border-t border-gray-300">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-lg font-bold text-gray-900">Rs. {item.amount.toFixed(2)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
